...
def main():
    ...
    data_path = os.path.join(cfg.output_dir, f"hard_negative_preference_pairs_{suffix}.json")
    ...
    model_path = os.path.join(cfg.output_dir, f"action_preference_model_hard_{suffix}.pt")
    torch.save(
        {
    ...
    save_json(
        os.path.join(cfg.output_dir, f"action_preference_train_history_hard_{suffix}.json"),
        history,
    )

    print(f"[INFO] saved hard-negative preference model -> {model_path}")
