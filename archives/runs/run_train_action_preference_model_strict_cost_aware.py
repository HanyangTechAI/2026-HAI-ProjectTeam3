...
def main():
    ...
    data_path = os.path.join(cfg.output_dir, f"strict_cost_aware_pairs_{suffix}.json")
    ...
    model_path = os.path.join(cfg.output_dir, f"action_preference_model_strict_cost_{suffix}.pt")
    torch.save(
        {
    ...
    save_json(
        os.path.join(cfg.output_dir, f"action_preference_train_history_strict_cost_{suffix}.json"),
        history,
    )

    print(f"[INFO] saved strict cost-aware preference model -> {model_path}")
