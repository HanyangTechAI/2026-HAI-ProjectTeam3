...
def main():
    ...
    data_path = os.path.join(cfg.output_dir, f"combined_preference_pairs_{suffix}.json")
    ...
    model_path = os.path.join(cfg.output_dir, f"action_preference_model_combined_{suffix}.pt")
    torch.save(
        {
    ...
    save_json(
        os.path.join(cfg.output_dir, f"action_preference_train_history_combined_{suffix}.json"),
        history,
    )

    print(f"[INFO] saved combined preference model -> {model_path}")
