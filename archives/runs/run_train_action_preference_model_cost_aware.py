...
def main():
    ...
    data_path = os.path.join(cfg.output_dir, f"cost_aware_hard_negative_pairs_{suffix}.json")
    ...
    model_path = os.path.join(cfg.output_dir, f"action_preference_model_cost_aware_{suffix}.pt")
    torch.save(
        {
    ...
    save_json(
        os.path.join(cfg.output_dir, f"action_preference_train_history_cost_aware_{suffix}.json"),
        history,
    )

    print(f"[INFO] saved cost-aware preference model -> {model_path}")
