...
def action_cost_tier_by_idx(action_idx: int) -> str:
    if action_idx in [0, 4]:
        return "low"
    if action_idx in [2, 11]:
        return "mid"
    if action_idx in [8, 10]:
        return "high"
    return "mid"
...
def required_margin_for_preferred(
    ...
    tier = action_cost_tier_by_idx(preferred_action_idx)
...
def main():
    ...
    base_margin = getattr(cfg, "strict_cost_base_margin", 0.0)
    extra_mid_margin = getattr(cfg, "strict_cost_mid_margin", 0.05)
    extra_high_margin = getattr(cfg, "strict_cost_high_margin", 0.10)
    ...
    for global_idx, rows in grouped.items():
        ...
        for better in correct_rows:
            preferred_idx = better["action_idx"]
            preferred_tier = action_cost_tier_by_idx(preferred_idx)

            required_margin = required_margin_for_preferred(
                preferred_action_idx=preferred_idx,
                ...
                strict_pairs.append(
                    {
                        ...
                        "preferred_cost_tier": preferred_tier,
    ...
    save_json(
        os.path.join(cfg.output_dir, f"strict_cost_aware_pairs_{suffix}.json"),
        strict_pairs,
    )
    save_json(
        os.path.join(cfg.output_dir, f"strict_cost_aware_best_action_dataset_{suffix}.json"),
        best_action_dataset,
    )
    ...
    save_json(
        os.path.join(cfg.output_dir, f"strict_cost_aware_summary_{suffix}.json"),
        summary,
    )
